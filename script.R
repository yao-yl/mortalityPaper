...
# read data
 dta <- read.dta13("listing+round1+round2_indivual_level_data.dta")
# the population without duplications, remove all "verified==0"
duplicate_id=which( dta$dd19_verified_r1==0 |  dta$dd20_verified_r1==0 )
cd=dta[-duplicate_id, ]
cd=cd[which(cd$survey_done_r1=="Complete" ), ]  #cd: the Complete Dataset 
dim(cd)  
dim(dta)  

# check the number:
length( which (cd$survey_done_r2=="Complete" & cd$dd20_verified_r1==1 )) # 133
length( which (cd$survey_done_r2=="Complete" & cd$dd20_verified_r2_==1 )) # 154
length( which (cd$survey_done_r2=="Complete" & cd$dd19_verified_r1==1 )) # 353
length( which (cd$survey_done_r2=="Complete" & cd$dd19_verified_r2==1 )) # 0


cd=cd[-which(cd$dd20_verified_r2_==0 & !is.na(cd$dd20_month_r1)), ]
## aggregrate age:  
...
summary(glm(dropout~age, family = binomial(link = "logit")))
summary(glm(dropout~sex, family = binomial(link = "logit")))
summary(glm(dropout~edu, family = binomial(link = "logit")))
summary(glm(y1~edu, family = binomial(link = "logit")))
dd=data.frame(case=case, age=age,male=male, edu_family=edu_family, t1=t1)

summary(glm(case~age+male+edu_family, data = dd[-which(dd$t1>"2020-03-01"),],
...
pdf("~/desktop/age_count.pdf", width=4, height=2)
layout(matrix(c(1,1,2,3), ncol=2 ) )
par(oma=c(1,1,1,0),mar=c(1,1.3,1,0.5) ,mgp=c(1.5,0.25,0), lwd=0.5,tck=-0.01, cex.axis=0.7, cex.lab=0.6, cex.main=0.9)
hist(ifelse(age<80,age,80), breaks = seq(0,80, by=5),  yaxs='i', xaxs='i', axes = FALSE, main="", xlab="", ylab="", col="#B97C7C", border = "#A25050")
abline(h=c(2500,5000,7500), col='gray60', lty=2)
axis(1, lwd=0.5, at=c(seq(0,80,by=20)[-5], 78),  labels = c(seq(0,80,by=20)[-5], "80+"),  padj=-0.8)
axis(2, lwd=0.5, at=c(0,5000,1e4),	c("0","5000",expression(10^4) ),  las=2)
mtext(1, line=0.5, text="age",cex=0.7)
mtext(3, line=0.5, text="survey population" ,cex=0.7)
mtext(2, line=1.5, text="household member count" ,cex=0.7)
box(lwd=0.5, bty='l')

hist(ifelse(	age[case==1& t1<as.Date("2020-01-01")]<80,	age[case==1&  t1<as.Date("2020-01-01")],80) , ylim=c(0,150), breaks = seq(0,80, by=5),  yaxs='i', xaxs='i', axes = FALSE, main="", xlab="", ylab="", col="#B97C7C", border = "#A25050")
abline(h=c(100,50), col='gray60', lty=2)
axis(1, lwd=0.5, at=c(seq(0,80,by=20)[-5], 78),  labels = c(seq(0,80,by=20)[-5], "80+"),  padj=-0.8)
axis(2, lwd=0.5, at=c(0,50,150,100),	  las=2)
mtext(3, line=0.5, text="deceased population, 2019" ,cex=0.7)
box(lwd=0.5, bty='l')
...
axis(2, lwd=0.5, at=c(0,50,150,100),	  las=2)
mtext(1, line=0.5, text="age",cex=0.7)
mtext(3, line=-0.8, text="2020" ,cex=0.7)
box(lwd=0.5, bty='l')
...
data_lab[1]="19/1"
data_lab[5]="20/1"
pdf("~/desktop/case_count_adj.pdf", width=2.5, height=2.5)
par(mfrow=c(1,1),oma=c(1,1,1,0),mar=c(1,1.5,1,0.5) ,mgp=c(1.5,0.25,0), lwd=0.5,tck=-0.01, cex.axis=0.7, cex.lab=0.6, cex.main=0.9)
...
mtext(1, line=0.5, text="month",cex=0.7)
mtext(3, line=0.5, text="surveyed mortality rate by month" ,cex=0.7)
mtext(2, line=0.5, text=expression(10^{-3}) ,cex=0.7, las=2, padj = -5)
mtext(2, line=1.5, text="monthly mortality rate" ,cex=0.7)
box(lwd=0.5, bty='l')
...
print(m1, pars="month_eff_excess")
mean(sss$month_eff_excess)
### visualize 
...
pdf("~/desktop/base_rate08.pdf", width =5.5, height=2)
par(mfrow=c(1,2),oma=c(1,1,1,0), pty='m',mar=c(1,1.5,1,1) ,mgp=c(1.5,0.25,0), lwd=0.5,tck=-0.01, cex.axis=0.7, cex.lab=0.6, cex.main=0.7) 
v=sss$age_eff 
plot(age_vec_cap,  colMeans(v),  axes=F,ylab="",xlab="",type='l',col="#8F2727",lwd=1.5,  xlim= c(0,82), xaxs='i', c(-11,-4))	
axis(2, las=2,lwd=0.5, at=seq(-4,-10,by=-2))
zz=apply(v, 2, quantile, c(0.975, 0.75, 0.25, 0.025))
points(age_vec_cap,  colMeans(v),  col="#8F2727",pch=19,cex=0.6)
polygon(x=c(age_vec_cap,rev(age_vec_cap)), y=c(zz[1,],rev(zz[4,] )  ), border=NA,col=  adjustcolor( "#B97C7C", alpha.f = 0.4), xpd=T)
polygon(x=c(age_vec_cap,rev(age_vec_cap)), y=c(zz[2,],rev(zz[3,] )  ), border=NA,col=  adjustcolor( "#B97C7C", alpha.f = 0.6), xpd=T)
...
mtext(3,text= "baseline mortality rate, \n log odds by age", cex=0.7,line=-0.5)
plot(1:12,  colMeans(sss$time_eff[,1:12]),  axes=F,ylab="",xlab="",type='l',col="#8F2727",lwd=1.5,  xlim= c(1,12), xaxs='i', ylim=c(-1,1))
axis(2, las=2,lwd=0.5 )
...
mtext(3,text= "seasonal trend, \n log odds", cex=0.7,line=-0.5)
dev.off()

pdf("~/desktop/excess_rate08.pdf", width = 5.6, height=2)
par(mfrow=c(1,2),oma=c(1,1,1,0), pty='m',mar=c(1,1.5,1,1) ,mgp=c(1.5,0.25,0), lwd=0.5,tck=-0.01, cex.axis=0.7, cex.lab=0.6, cex.main=0.7) 
draw_base_rate(sss$covid_eff, yl=c(-1.2,1.2))	
abline(h=c(0.5,0,-0.5),col= 1, lty=2, lwd=0.5 )
mtext(3,text= "excess mortality rate, \n female, log odds", cex=0.7,line=-0.5)
temp=sss$covid_eff 
for(i in 1:ncol(temp))
	temp[,i]=	temp[,i]+sss$covid_male
draw_base_rate(temp , yl=c(-1.2,1.2))	
abline(h=c(0.5,0,-0.5),col= 1, lty=2, lwd=0.5 )
mtext(3, text="excess mortality rate, \n male, log odds", cex=0.7,line=-0.5)
dev.off()



pdf("~/desktop/coef08.pdf", width = 3, height=3)
par(mfrow=c(1,1),oma=c(1,1,1,0), pty='m',mar=c(1,1.5,1,1) ,mgp=c(1.5,0.25,0), lwd=0.5,tck=-0.01, cex.axis=0.7, cex.lab=0.6, cex.main=0.7) 
plot(c(0,0),type='n', axes=F,ylab="",xlab="",xaxs='i', xpd=T, 
		 xlim=c(0.5, 5.8), ylim=c(-1,1))
abline(h=c(-.5,.5,0), col='gray40', lty=2)
...
text(x= c(1, 2.5, 4, 5), y=c(0.6, 0.8, 0.4, -0.5), col= 1, labels = c("male\n baseline", "edu\n baseline", "male\n excess", "edu\n excess") , cex=0.75)
dev.off()
